package com.doubleskyline.core.config.mvc;
...
public class MultiRequestBodyArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    private ServletModelAttributeMethodProcessor servletModelAttributeMethodProcessor = new ServletModelAttributeMethodProcessor(false);
    ...
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
        ...
        if(StringUtils.isBlank(jsonBody)){
            ...
            if(null != parameterMap && !parameterMap.isEmpty()){
                Object result = servletModelAttributeMethodProcessor.resolveArgument(parameter, mavContainer, webRequest, binderFactory);
                if(null != result){
                    return result;
                }
            }
            ...
            if (parameterAnnotation.required()) {
                throw new IllegalArgumentException(String.format("required body %s is null", parameterName));
            }
            return null;
        }
        ...
        if (StringUtils.isNotEmpty(key)) {
            ...
            // 如果设置了value但是解析不到，报错
            if (valueNode == null && parameterAnnotation.required()) {
        ...
        if (valueNode != null) {
            ...
            if (parameterType.isPrimitive()) {
                return parsePrimitive(parameterType.getName(), valueNode);
            }
            // 基本类型包装类
            if (isBasicDataTypes(parameterType)) {
                return parseBasicTypeWrapper(parameterType, valueNode);
                // 字符串类型
            } else if (parameterType == String.class) {
                return valueNode.toString();
            }
            // 其他复杂对象
            return  objectMapper.readValue(valueNode.toString(), objectMapper.constructType(parameterType));
        }
        ...
        // 非基本类型，允许解析，将外层属性解析
        Object result = objectMapper.readValue(jsonNode.toString(), objectMapper.constructType(parameterType));
     ...
     * 基本类型解析
     */
    private Object parsePrimitive(String parameterTypeName, JsonNode node) {
