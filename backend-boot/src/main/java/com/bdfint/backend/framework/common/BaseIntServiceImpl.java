 ...
 *
 */
public abstract class BaseIntServiceImpl<T extends BaseIntEntity<T>> implements BaseIntService<T> {
	...
	public PageInfo<T> getPage(T object, Example example) throws Exception {
        if (example.getOredCriteria().size() > 0) {
            ...
            for(Example.Criteria criteria: oredCriteria){
                criteria.andNotEqualTo("delFlag", BaseIntEntity.DEL_FLAG_DELETE);
            }
        } else {
